...
export async function GET(req:NextRequest) {
    const cookie = req.cookies.get("username")?.value || "No cookie found";
    ...
    try {
        ...
        if (rows.length > 0){
            ...
            if (isPasswordValid) {
                ...
                const response = NextResponse.json({ message: 'User signed in successfully', data: {token,"username":user.username} }, { status: 200 });
                response.cookies.set('token', token, { httpOnly: true, secure: false, sameSite: 'strict', maxAge: 3600 });
                response.cookies.set('username', user.username, { httpOnly: true, secure: false, sameSite: 'strict', maxAge: 3600 });
