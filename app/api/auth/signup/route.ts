  ...
  try {
    ...
    if (!username || !password) {
      return NextResponse.json(
        { message: "email and password are required", data: null } as signupresponse,
        { status: 400 }
      );
    }
    const result = await registerUser(username, password);
    return NextResponse.json(
      { message: result.message, data: null } as signupresponse,
      { status: result.statuscode }
    );
  } catch (err) {
    console.error("error came when :", err);
    return NextResponse.json(
      { message: "server error", data: null } as signupresponse,
      { status: 500 }
    );
  }
}
